...
public class Jeu{
    ...
    void ecrit(PrintWriter pw) {
		...
		pw.println(l1);
		for(int i=0; i<largeur*largeur;i++)
			l2 += "  "+Triominos.onechar(tab[i].c) + " "+Triominos.onechar(tab[i].b)+ " ";
		pw.println(l2);
		for(int i=0; i<largeur*largeur;i++)
			l3 += " /_" + Triominos.onechar(tab[i].a) + "_\\";
		pw.println(l3);
    }
    ...
    void affiche() {
      for(int i=0; i<largeur*largeur;i+=12){
        ...
        for(j2 = i ; (j2 < largeur*largeur) && (j2 < i+12) ; j2++)
          System.out.print("  "+Triominos.onechar(tab[j2].c) + " "+Triominos.onechar(tab[j2].b)+ " ");
        System.out.println("");
